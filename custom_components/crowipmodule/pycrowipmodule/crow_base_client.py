...
class CrowIPModuleClient(asyncio.Protocol):
    ...
    def start(self):
        ...
        _LOGGER.debug("Client Start requested")
        self._shutdown = False
    ...
    def stop(self):
        ...
        _LOGGER.debug("Client Stop requested")
        self._connected = False
        self._shutdown = True
    ...
    async def connect(self):
        ...
        # Lambda fix for create_connection factory
        factory = lambda: self
        ...
        try:
            ...
            _LOGGER.debug(f'Timeout or error connecting to Crow IP module: {e}')
            self.handle_connect_failure()

    def connection_made(self, transport):
        """asyncio callback for a successful connection."""
        _LOGGER.info("Connection made to Crow IP Module!")
        ...
        if self._connected:
            _LOGGER.debug("Sending initial STATUS command")
    ...
    async def reconnect(self, delay):
        ...
        _LOGGER.debug(f"Reconnecting in {delay} seconds...")
        self.disconnect()
    ...
    def data_received(self, data):
        ...
        if data != '':
            try:
                ...
                _LOGGER.error(f'Received invalid message (Decode error): {e}. Skipping.')
                return

            for line in lines:
                if not line: continue
                _LOGGER.debug(f"RX LINE: '{line}'")
                
                parsed = self.parseHandler(line)
                
                # If parseHandler returned empty dict, no match was found
                if not parsed:
                    continue
    ...
    def parseHandler(self, rawInput):
        ...
        # _LOGGER.debug(f'Parsing response line: {rawInput}')
        result = {}
        if rawInput != '':
            match_found = False
    ...
    async def keep_alive(self):
        ...
        while not self._shutdown:
            if self._connected:
                # _LOGGER.debug("Sending Keepalive (STATUS)")
                self.send_command('status', '')
    ...
    def disarm(self, code):
        ...
        # self.send_command('status', '') # Often helpful to request status immediately after
    ...
    def handle_connect_failure(self):
        ...
        _LOGGER.error('Handle Connect Failure called.')
        self._connected = False
        if not self._shutdown:
            _LOGGER.info('Scheduling reconnect...')
